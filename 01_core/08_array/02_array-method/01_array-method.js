 ...
 * includes : 배열에 해당 요소가 포함 하는 지 여부 리턴
 */
...
console.log(`includes(존재하지 않는 요소) : ${foodList.includes("햄버거")}`);
...
const backend = ["자바", "스프링", "서블렛", "마이바티스"];
// 잘라 반환을 해주는 것이지, 요소를 삭제하는 것은 아니다.
console.log(`slice(1, 3) : ${backend.slice(1, 3)}`);
console.log(backend);
// (index, 제거수, 추가값1, 추가값2, ...)
console.log(`splice(3, 1) : ${backend.splice(3, 1, "jdbc")}`);
console.log(backend);
// 배열의 순서 뒤집기
